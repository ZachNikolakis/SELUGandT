...
CONFIG_PATH="/work/zach/clownfish_genome/long_reads.config"
CMD_PATH="/work/zach/MaSuRCA-3.2.4/bin/masurca"
...
#}
export PATH="/work/zach/MaSuRCA-3.2.4/bin:/work/zach/MaSuRCA-3.2.4/bin/../CA/Linux-amd64/bin:$PATH"
save PATH
export PERL5LIB=/work/zach/MaSuRCA-3.2.4/bin/../lib/perl${PERL5LIB:+:$PERL5LIB}
save PERL5LIB
NUM_THREADS=16
save NUM_THREADS
...
rename_filter_fastq 'pe' <(exec expand_fastq 'data/paired-end/SRR6255796_trimmed_1P' | awk '{if(length($0>250)) print substr($0,1,250); else print $0;}') <(exec expand_fastq 'data/paired-end/SRR6255798_trimmed_1P' | awk '{if(length($0>250)) print substr($0,1,250); else print $0;}' ) > 'pe.renamed.fastq'
echo 'se 150 7' >> meanAndStdevByPrefix.pe.txt
rename_filter_fastq 'se' <(exec expand_fastq 'data/single-end/SRR6266482_trimmed.fastq' | awk '{if(length($0>250)) print substr($0,1,250); else print $0;}') '' > 'se.renamed.fastq'
...
quorum_error_correct_reads  -q $((MIN_Q_CHAR + 40)) --contaminant=/work/zach/MaSuRCA-3.2.4/bin/../share/adapter.jf -m 1 -s 1 -g 1 -a 3 -t 16 -w 10 -e 3 -M  quorum_mer_db.jf pe.renamed.fastq se.renamed.fastq --no-discard -o pe.cor --verbose 1>quorum.err 2>&1 || {
...
/work/zach/MaSuRCA-3.2.4/bin/mega_reads_assemble_cluster.sh -Pb 300000000 -q all.q -G 0 -C 30 -t 16 -e $ESTIMATED_GENOME_SIZE -m work1 -p data/raw_data_nanopore/mega_fastq -a /work/zach/MaSuRCA-3.2.4/bin/../CA8/Linux-amd64/bin -o "   cgwErrorRate=0.15" -B 17 -D 0.029
